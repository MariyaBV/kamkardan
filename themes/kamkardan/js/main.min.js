const callBackBlocks=document.querySelectorAll(".block-callback-form");callBackBlocks.forEach((e=>{const t=e.getAttribute("id");document.getElementById(t).addEventListener("submit",(function(e){e.preventDefault();var a=document.getElementById("callback-name-"+t).value,l=document.getElementById("callback-phone-"+t).value,n=document.getElementById("callback-type-"+t).value,c=new FormData;c.append("name",a),c.append("phone",l),c.append("type",n),c.append("action","callback_request"),fetch("/wp-admin/admin-ajax.php",{method:"POST",body:c}).then((e=>e.json())).then((e=>{alert("Заявка отправлена! Имя: "+a+", Телефон: "+l),document.getElementById("callbackForm").style.display="none"})).catch((e=>{console.error("Ошибка:",e),alert("Произошла ошибка при отправке заявки.")}))}))}));
document.getElementById("callbackButtonVacancy").addEventListener("click",(function(){document.getElementById("callbackFormVacancy").style.display="block"})),document.getElementById("closeCallbackFormVacancy").addEventListener("click",(function(){document.getElementById("callbackFormVacancy").style.display="none"}));
